...
const UsersTableResults = (props)=> {
  ...
  useEffect(()=> {
    console.log(filter);
    usersSearchCall(filter).then(results => setResults(results));
  }, [filter]);

  useEffect(()=> console.log(results), [results]);

  const watchUser = id => history.push({pathname: '/user', state:{id}});
        ...
        <Table.Body>
            {
              results.map(item => (
                <Table.Row key={item.id}>
                  ...
                  <Table.Cell>
                    <Button icon color='green' onClick={()=> watchUser(item.id)}>
                      <Icon name='eye' />
                    </Button>
